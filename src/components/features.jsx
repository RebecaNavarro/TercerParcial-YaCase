import React from "react";

export const Features = (props) => {
  // Determinar cuántos elementos hay y cómo distribuirlos
  const itemCount = props.data ? props.data.length : 0;
  const needsCentering = itemCount > 0 && itemCount < 4;

  // Calcular las clases de columnas según la cantidad
  const getColumnClass = () => {
    if (itemCount === 1) return "col-xs-12 col-md-6 col-md-offset-3";
    if (itemCount === 2) return "col-xs-12 col-md-6";
    if (itemCount === 3) return "col-xs-12 col-md-4";
    return "col-xs-6 col-md-3"; // 4 o más elementos
  };

  return (
    <div id="features" className="text-center">
      <div className="container">
        <div className="col-md-10 col-md-offset-1 section-title">
          <h2>{props.title}</h2>
        </div>
        <div className="row">
          {props.data
            ? props.data.map((d, i) => (
                <div 
                  key={`${d.title}-${i}`} 
                  className={getColumnClass()}
                  style={
                    needsCentering 
                      ? {
                          paddingBottom: '30px',
                          marginBottom: '20px',
                          display: 'flex',
                          flexDirection: 'column',
                          alignItems: 'center'
                        }
                      : {
                          paddingBottom: '20px'
                        }
                  }
                >
                  <i className={d.icon} style={needsCentering ? { marginBottom: '15px' } : {}}></i>
                  <h3>{d.title}</h3>
                  <p style={needsCentering ? { textAlign: 'center' } : {}}>{d.text}</p>
                  <a href={d.link} target="_blank" rel="noopener">
                    Ir al documento
                  </a>
                </div>
              ))
            : "Loading..."}
        </div>
      </div>
    </div>
  );
};